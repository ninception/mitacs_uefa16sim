...
findelo <- function(tm){
    ...
    return(as.double(tmelo))
}

# function to find prob. of winning from qual table
modprob <- function(team1 , team2) {
    pvec <- numeric(length = 3)
    v1 <- rep(0,4) # P,W,D,L
    v2 <- rep(0,4)
    ...
    for(i in 1:24){
        ...
        }else if(allmat[i,1] == team2){
            v2 <- allmat[i,2:5]
        }
    }
    
    pvec[1] <- (v1[2] + v2[4]) / (v1[1] + v2[1]) #win1
    pvec[2] <- (v1[4] + v2[2]) / (v1[1] + v2[1]) #win2
    pvec[3] <- (v1[3] + v2[3]) / (v1[1] + v2[1]) #draw
...
findprob <- function(team1, team2){
    ...
    elo1 <- findelo(team1)
    elo2 <- findelo(team2)
    pvec <- modprob(team1,team2)
    #now scale
    pvec[1] <- pvec[1] * (elo1/(elo1+elo2)) 
    pvec[2] <- pvec[2] * (elo2/(elo1+elo2))
    return(as.double(pvec)) 
}
...
rankvect <- function(wvect){
    l1 <- length(wvect)
    rvect <- vector(mode = "numeric", length = l1)
    for(i in 1:l1){
        rvect[i] <- findrank(wvect[i]) 
    }
    return(rvect)
}

# use findpts(function)
ptsvect <- function(wvect){
    l1 <- length(wvect)
    pvect <- vector(mode = "numeric", length = l1)
    for(i in 1:l1){
        pvect[i] <- findpts(wvect[i]) 
    }
    return(pvect)
}


pwin <- vector(mode = "numeric", length = numsim) #winner points
rwin <- vector(mode = "numeric", length = numsim) #winner rank

psmw <- vector(mode = "numeric", length = 2*numsim) #finalist points
rsmw <- vector(mode = "numeric", length = 2*numsim) #finalist rank

pqrw <- vector(mode = "numeric", length = 4*numsim) #semi-finalists points
rqrw <- vector(mode = "numeric", length = 4*numsim) #semi-finalists rank

prsw <- vector(mode = "numeric", length = 8*numsim) #quarter-finalists points
rrsw <- vector(mode = "numeric", length = 8*numsim) #quarter-finalists rank
...
for(k in 1:numsim) {
    ...
    pwin[k] <- findpts(finalwin)
    rwin[k] <- findrank(finalwin)
    
    j1 <- 2*k
    psmw[j1-1] <- ptsvect(semwin)[1]
    psmw[j1] <- ptsvect(semwin)[2]
    
    rsmw[j1-1] <- rankvect(semwin)[1]
    rsmw[j1] <- rankvect(semwin)[2]
    
    j2 <- 4*k
    pqrw[j2-3] <- ptsvect(qrtwin)[1]
    pqrw[j2-2] <- ptsvect(qrtwin)[2]
    pqrw[j2-1] <- ptsvect(qrtwin)[3]
    pqrw[j2] <- ptsvect(qrtwin)[4]
    
    rqrw[j2-3] <- rankvect(qrtwin)[1]
    rqrw[j2-2] <- rankvect(qrtwin)[2]
    rqrw[j2-1] <- rankvect(qrtwin)[3]
    rqrw[j2] <- rankvect(qrtwin)[4]
    
    j3 <- 8*k
    prsw[j3-7] <- ptsvect(r16win)[1]
    prsw[j3-6] <- ptsvect(r16win)[2]
    prsw[j3-5] <- ptsvect(r16win)[3]
    prsw[j3-4] <- ptsvect(r16win)[4]
    prsw[j3-3] <- ptsvect(r16win)[5]
    prsw[j3-2] <- ptsvect(r16win)[6]
    prsw[j3-1] <- ptsvect(r16win)[7]
    prsw[j3] <- ptsvect(r16win)[8]
    
    rrsw[j3-7] <- rankvect(r16win)[1]
    rrsw[j3-6] <- rankvect(r16win)[2]
    rrsw[j3-5] <- rankvect(r16win)[3]
    rrsw[j3-4] <- rankvect(r16win)[4]
    rrsw[j3-3] <- rankvect(r16win)[5]
    rrsw[j3-2] <- rankvect(r16win)[6]
    rrsw[j3-1] <- rankvect(r16win)[7]
    rrsw[j3] <- rankvect(r16win)[8]
    ...
    for(i in 1:24) {
        ...
        if(resqual == 1){
            onlyqual2[i] <- onlyqual2[i] + 1
        }
    }
    
    rm(list = "sim1", "finpts", "cmatrix", "goalscr", "goaldif", "finalpos", "fint3", "fint3ids", "gsqual", "finaltem", "r16win", "qrtwin", "semwin", "finalwin")
    gc()
}

# RENAME RData file to be saved before running the program!! ####

save(list = c("pwin", "psmw", "pqrw", "prsw", "rwin", "rsmw", "rqrw", "rrsw","onlyqual2") , file = "./Nptsqual1.RData")
